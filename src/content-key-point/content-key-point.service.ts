...
export class ContentKeyPointService {
 ...
 async create(countryId: string, createContentKeyPointDto: CreateContentKeyPointDto) {
    try {
      ...
      throw new BadRequestException('Failed to create content key point: ' + error.message);
    }
  }
  ...
  async findAll() {
    try {
      ...
      throw new BadRequestException('Failed to retrieve content key points: ' + error.message);
    }
  }
  ...
  async findOne(id: number) {
    try {
      ...
      throw new BadRequestException('Failed to retrieve content key point: ' + error.message);
    }
  }

  async update(id: number, updateContentKeyPointDto: UpdateContentKeyPointDto) {
    try {
      const existingContentKeyPoint = await this.prisma.contentKeyPoint.findUnique({
        where: { id },
      });
      ...
      throw new BadRequestException('Failed to update content key point: ' + error.message);
    }
  }
  ...
  async remove(id: number) {
    try {
      ...
      throw new BadRequestException('Failed to delete content key point: ' + error.message);
    }
  }
}